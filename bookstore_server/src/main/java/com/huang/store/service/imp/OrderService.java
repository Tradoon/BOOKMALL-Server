...
public interface OrderService {
    ...
    int userDelOrder(Long id);//用户删除订单,这里的删除不是真正的删除，而是设置删除标志位为true

    int batchDelOrder(List<Integer> item);//批量删除 这里涉及多表联合删除

    int modifyOrderStatus(Long id, String orderStatus);//修改订单的状态

    int deliverBook(Long id,int logisticsCompany,String logisticsNum);//发货
